...
//!!!	static string iconv_convert (iconv_t handle, string s);
	static string _time_pid_prefix ();
public:
	static unsigned long inet_aton (string address);
	static string inet_ntoa (unsigned long address);
	static string ultostr (unsigned long value, int padsize = 0);
	static unsigned long strtoul (string value);
	static c_path string2path (string value);
	static string path2string (c_path value);
	static string time2string (time_t value);
	static time_t string2time (string value);
	static string size2string (size_t value);
	static size_t string2size (string value);
...
//!!!	static void iconv_free ();
	...
	// streaming routies
	static        string  readline  (int fd, timeval * timer, t_stream_status * status, string::value_type terminator, bool strict, bool skipempty);
	static vector<string> readblock (int fd, timeval * timer, t_stream_status * status, string::value_type terminator, bool strict, bool skipempty, unsigned mincount = 0, unsigned maxcount = 0);
	...
	static          void log        (vector<string> message);
	static          void debug      (string         message);
	static          void debug      (vector<string> message);

	// forking routines
	static pid_t fork (t_fork_func function, t_void_func init = NULL, t_void_func free = NULL);
	static pid_t exec (char * cmd, char **        arg, char **        env, map<int,int> fds);
	static pid_t exec (string cmd, vector<string> arg, vector<string> env, map<int,int> fds);
